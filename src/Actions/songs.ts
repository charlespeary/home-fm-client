import { Album, StandardAction, Action, Song, SongsRequestData } from "./index";
...
export async function fetchUserSongs(paginationStatus: PaginationStatus) {
    ...
    let total = 0;
    const songs = await axios.get<SongsRequestData>(`/me/tracks?limit=50&offset=${paginationStatus.offset}`, {
        headers: {
    ...
    }).catch(e => { return { error: true } as Result<Song[]> });
...
export async function getUserFavouriteSongs(dispatch: Dispatch<AnyAction>) {
    let paginationStatus: PaginationStatus = { total: 0, offset: 0, firstFetch: true };
    let songs: Song[] = [];

    // fetch songs until all of 
    while (paginationStatus.offset < paginationStatus.total || paginationStatus.firstFetch) {
        ...
        if (songsChunk.songs.error) {
            ...
            paginationStatus = { offset: songsChunk.offset, total: songsChunk.total, firstFetch: false };
